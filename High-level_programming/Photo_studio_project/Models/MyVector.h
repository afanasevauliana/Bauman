...
public:
    ...
    MyVector(size_t size) : m_size(size), m_capacity(size) {
        ...
        for (size_t i = 0; i < m_size; ++i) {
            new (&m_array[i]) T(); // Требуется конструктор по умолчанию
        }
    }
    ...
    ~MyVector() {
        for (size_t i = 0; i < m_size; ++i) {
            m_array[i].~T(); // Явно вызываем деструктор
        }
        operator delete[](m_array); // Освобождаем сырую память
    }
    ...
    void push_back(const T& value) {
        if (m_size >= m_capacity) {
            ...
            for (size_t i = 0; i < m_size; ++i) {
                ...
                m_array[i].~T(); // Уничтожаем старые объекты
            }
            operator delete[](m_array); // Освобождаем старую память
            m_array = new_array;
            m_capacity = new_capacity;
            m_end = m_array + m_size;
        }
        new (&m_array[m_size]) T(value); // Placement new для нового элемента
        ++m_size;
        m_end = m_array + m_size;
    }
    ...
    void erase(size_t index) {
        ...
        --m_size;
        m_array[m_size].~T(); // Уничтожаем последний элемент
        m_end = m_array + m_size;
    }
